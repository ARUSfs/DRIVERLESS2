...
CanInterface::CanInterface() : Node("can_interface"){
    ...
    const std::unordered_map<std::string, double*> variableMap = {
        // IMUData Fields
        {"IMUData.linear_acceleration.x", &IMUData.linear_acceleration.x},
        {"IMUData.linear_acceleration.y", &IMUData.linear_acceleration.y},
        {"IMUData.linear_acceleration.z", &IMUData.linear_acceleration.z},
        {"IMUData.angular_velocity.x", &IMUData.angular_velocity.x},
        {"IMUData.angular_velocity.y", &IMUData.angular_velocity.y},
        {"IMUData.angular_velocity.z", &IMUData.angular_velocity.z},

        // GPSData Fields
        {"GPSData.latitude", reinterpret_cast<double*>(&GPSData.latitude)}, // double field
        {"GPSData.longitude", reinterpret_cast<double*>(&GPSData.longitude)}, // double field
        {"GPSData.altitude", reinterpret_cast<double*>(&GPSData.altitude)}, // double field

        // GPS Velocity Fields
        {"GPSVelData.x", &GPSVelData.x},
        {"GPSVelData.y", &GPSVelData.y},
        {"GPSVelData.z", &GPSVelData.z},

        // Steering Angle
        {"steeringAngleData.data", reinterpret_cast<float*>(&steeringAngleData.data)}, // float field

        // RESData
        {"RESData.data", reinterpret_cast<float*>(&RESData.data)}, // float field

        // Inverted Speed
        {"InvSpeedData.data", &InvSpeedData.data} // float field
    };

    publisherMap = { 
        {"motorSpeedPub", motorSpeedPub},
        {"ASStatusPub", ASStatusPub},
        {"GPSPub", GPSPub},
        {"GPSSpeedPub", GPSSpeedPub},
        {"IMUPub", IMUPub},
        {"steeringAnglePub", steeringAnglePub},
        {"RESRangePub", RESRangePub}
    };
...
void parseMsg(const struct can_frame& frame, const CANParseConfig& config) {
    ...
    if (numBytes == 1) { 
        ...
        if (config.endianness == "LE") {
            ...
            rawValue = (frame.data[config.startByte] << 8) | frame.data[config.endByte];
        }
    } 
    else if (numBytes == 3) { 
        if (config.endianness == "LE") {
            ...
            rawValue = (frame.data[config.startByte] << 16) | 
                       (frame.data[config.startByte + 1] << 8) | 
                       frame.data[config.startByte + 2];
        }
    } 
    else if (numBytes == 4) { 
        if (config.endianness == "LE") {
            ...
            rawValue = (frame.data[config.startByte] << 24) |
                       (frame.data[config.startByte + 1] << 16) |
                       (frame.data[config.startByte + 2] << 8) |
                       frame.data[config.startByte + 3];
        }
    } 
    else {
