...
namespace GroundRemove 
{
	...
	void RemoveGround(pcl::PointCloud<PointXYZIRingTime>& cloudIn,
		...
		std::vector<std::vector<pcl::PointCloud<PointXYZIRingTime>::Ptr>> grid(num_rings, std::vector<pcl::PointCloud<PointXYZIRingTime>::Ptr>(num_sectors));

		// Create the grid
		for (int r = 0; r < num_rings; ++r) {
			for (int s = 0; s < num_sectors; ++s) {
				grid[r][s] = pcl::PointCloud<PointXYZIRingTime>::Ptr(new pcl::PointCloud<PointXYZIRingTime>());
			}
		}
