...
Perception::Perception() : Node("Perception")
{
    ...
    this->declare_parameter<double>("intensity_threshold", 0.4);
    this->declare_parameter<bool>("accumulation", true);
    ...
    this->declare_parameter<double>("estimation_time", 0.0);
    ...
    this->get_parameter("intensity_threshold", kIntensityThreshold);
    ...
    this->get_parameter("estimation_time", kEstimationTime);
    ...
    filtered_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(
        "/perception_acc/filtered_cloud", 10);
    ...
    pcl::PointCloud<PointXYZIRingTime>::Ptr cloud_filtered, std::vector<PointXYZColorScore>& clusters_centers)
{
    for (int i = cluster_indices.size() - 1; i >= 0; i--)
    {
        ...
        if ((max_z - min_z) < 0.10 || (max_z - min_z) > 0.5 || (max_x - min_x) > 0.5 || (max_y - min_y) > 0.5)
        {
...
void Perception::lidar_callback(const sensor_msgs::msg::PointCloud2::SharedPtr lidar_msg)
{
    ...
    if (this->now().seconds() - start_time < kEstimationTime) {
        RCLCPP_INFO(this->get_logger(), "Less than %d seconds have passed, skipping.", kEstimationTime);
        return;
    }
    ...
    if(kAccumulation) Accumulation::accumulate(cloud, accumulated_cloud, x_, y_, yaw_, kDistanceLidarToCoG, kBufferSize);
    else *accumulated_cloud = *cloud;
        
    if (kDebug) RCLCPP_INFO(this->get_logger(), "Accumulated cloud size: %zu", accumulated_cloud->size());
    ...
    if (kDebug) RCLCPP_INFO(this->get_logger(), "Cropping Time: %f", this->now().seconds() - time_start);
    ...
    if (kDebug) RCLCPP_INFO(this->get_logger(), "Ground Filter Time: %f", this->now().seconds() - time_start);
    ...
    fec.setQuality(0.75);
    fec.setMinClusterSize(3);     
    fec.setMaxClusterSize(200);

    fec.segment(cluster_indices);
    
    if (kDebug) RCLCPP_INFO(this->get_logger(), "Clustering time: %f", this->now().seconds() - time_start);
    ...
    if (kDebug) RCLCPP_INFO(this->get_logger(), "Number of posibles cones: %zu", clusters_centers.size());


    //Recover ground points
    Perception::reconstruction(cloud_plane, cloud_filtered, cluster_indices, clusters_centers, kRadius);
    if (kDebug) RCLCPP_INFO(this->get_logger(), "Reconstruction time: %f", this->now().seconds() - time_start);


    //Filter clusters by size
    Perception::filter_clusters(cluster_indices, cloud_filtered, clusters_centers);
    if (kDebug) RCLCPP_INFO(this->get_logger(), "Filtering time: %f", this->now().seconds() - time_start);
    ...
    // Convert cluster_points to type PointXYZIRingTime para scoring
    std::vector<pcl::PointCloud<PointXYZIRingTime>::Ptr> cluster_points_i;
    for (auto &cluster : cluster_points) {
        pcl::PointCloud<PointXYZIRingTime>::Ptr cluster_i(new pcl::PointCloud<PointXYZIRingTime>);
        for (const auto &p : cluster->points) {
            PointXYZIRingTime pt;
    ...
    if (kDebug) RCLCPP_INFO(this->get_logger(), "Scoring time: %f", this->now().seconds() - time_start);
