...
Perception::Perception() : Node("perception_acc")
{
    ...
    car_state_sub_ = this->create_subscription<common_msgs::msg::State>(
        "/car_state/state", 10, std::bind(&Perception::car_state_callback, this, std::placeholders::_1));
...
void Perception::car_state_callback(const common_msgs::msg::State::SharedPtr state_msg)
{
    vx_ = state_msg->vx;
    vy_ = state_msg->vy;
    r_ = state_msg->r;
...
{   
    ...
    transform_matrix.block(0, 0, 2, 2) << std::cos(phi), -std::sin(phi), std::sin(phi), std::cos(phi);
    transform_matrix(0, 3) = vx_*dt_;
    transform_matrix(1, 3) = vx_*dt_;
    ...
    pcl::PointCloud<PointXYZIRingTime>::Ptr global_cloud2(new pcl::PointCloud<PointXYZIRingTime>);
    *global_cloud2 += *last_cloud_;
    *global_cloud2 += *updated_cloud;
...
// Publish
    if (DEBUG) {
        ...
        pcl::toROSMsg(*global_cloud2, acum_msg);
