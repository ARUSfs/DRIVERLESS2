...
void SkidpadPlanning::perception_callback(sensor_msgs::msg::PointCloud2::SharedPtr per_msg) {
    ...
    if(ransac_counter <1 && this->now().seconds() - start_time_2.seconds() >10 ){
        
        aux_straight_trajectory();

    }else if((this->now().seconds() - start_time_.seconds() < kPlanningTime )){
        ...
        if (!trajectory_calculated_) {
            ...
            if (ratio<0.7){
                RCLCPP_INFO(this->get_logger(), "WARNING! ratio (%f) is under threshold", ratio);
                 compute_ransac_centers();
...
void SkidpadPlanning::calculate_aux_profiles() {
    double step = 0.1;  
    ...
    for (size_t i = 1; i < s_.size(); ++i) {
        double ds = step;
        if(s_[i] < 10){
            speed_profile_[i] = std::min(1.0, sqrt(pow(speed_profile_[i - 1], 2) + 2 * 5 * ds));
        } else {
            speed_profile_[i] = std::max(0.0, sqrt(pow(speed_profile_[i - 1], 2) - 2 * 5 * ds));
        }
    }
    ...
    void SkidpadPlanning::publish_aux_trajectory() {
        common_msgs::msg::Trajectory trajectory_msg;
        double step = 0.1;
    ...
    void SkidpadPlanning::aux_straight_trajectory(){
        calculate_aux_profiles();
        ...
        auto t0 = this->now();
        while (this->now().seconds() - t0.seconds()<3)
        {
            publish_aux_trajectory();
        }
        ransac_counter++;
