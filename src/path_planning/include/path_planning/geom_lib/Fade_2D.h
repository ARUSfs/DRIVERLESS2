...
/** \brief Draws the triangulation as postscript or pdf file.
*
...
* custom colors.
*
* @param filename is the output name, i.e. "file.ps" or "file.pdf"
...
/** \brief Draws the triangulation using an existing Visualizer2 object
*
...
* @param bWithConstraints specifies if constraint segments shall be shown (default: true)
*
* @note When the last elements have been added, you need to finalize pVis with
* pVis->writeFile().
*/
...
*
* showVtk() draws a whole triangulation.
*
* @param filename The output filename, e.g., "file.vtk"
* @param color The color for the triangles
* @param constraintColor The color for the constraint edges. Use VTK_TRANSPARENT to suppress them
*
* @note It is also possible to use the VtkWriter class directly to draw custom
...
/** \brief Draws the triangulation using an existing VtkWriter
 *
 * @param pVtkWriter A pointer to a VtkWriter object that may already contain
 * geometric primitives or that may later be used to draw further elements
 * @param color The color for the triangles
 * @param constraintColor The color for the constraint edges. Use VTK_TRANSPARENT to suppress them
 * @note When the last elements have been drawn, you need to call pVtkWriter->writeFile()
 */
 ...
 *
 * @return the median 2.5D edge length of all edges in the triangulation.
 */
...
* triangles automatically. This robust version is very fast, and due to
* the additional checks, it is also very safe. Thus, it is highly recommended
* to use it always rather than the classic importTriangles() method.
*
* \param vPoints contains the input triangulation (3 subsequent points per triangle).
* \return a pointer to a Zone2 object or NULL if the input data is invalid.
*
